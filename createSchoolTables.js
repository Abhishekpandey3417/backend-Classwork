...
const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS complain_principal (
    id INT PRIMARY KEY AUTO_INCREMENT,
...
for (const dbName of databases) {
    ...
    const db = getSchoolDb(dbName);

    db.connect((err) => {

        if (err) {
            console.log(`Connection failed: ${dbName}`, err);
            return;
        }
        ...
        tables.forEach((query, index) => {
            ...
            db.query(query, (err) => {
                ...
                if (err) {
                    console.log(
                        `${dbName} table ${index + 1} error`,
                        err
                    );
                } else {
                    console.log(
                        `${dbName} table ${index + 1} created`
                    );
                }
            });
